...
_pkg_config = {
    ...
    'pyqt_config_args':  '--confirm-license --verbose -q /usr/bin/qmake-qt4 -c -j 10 LIBDIR_QT=/usr/lib STRIP= MOC=moc-qt4 LIBS_OPENGL= LIBS_X11= LIBS_THREAD= CFLAGS= -g -O2 -fstack-protector --param=ssp-buffer-size=4 -Wformat -Werror=format-security  -D_FORTIFY_SOURCE=2 CXXFLAGS= -g -O2 -fstack-protector --param=ssp-buffer-size=4 -Wformat -Werror=format-security  -D_FORTIFY_SOURCE=2 LFLAGS= -Wl,-Bsymbolic-functions -Wl,-z,relro -Wl,-O1 -m /usr/lib/python2.7/config-i386-linux-gnu -d /usr/lib/python2.7/dist-packages --dbus /usr/include/dbus-1.0',
